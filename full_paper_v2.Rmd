...
All variables apart from negative and total life events were approximately normally distributed (based on visual inspection, see supplementary table ...) and required no further transformation prior to the median split. 
...
Including the arcs between the same variables from X_1 $\rightarrow$ X_2 did not provide any theoretically meaningful information to the network structure as the majority of participants would be either be in a "Low" or "High" state for each pair of variables in the network. 
...
The use of differential equations to model changes in variables over time is a common procedure in BN analysis when there are repeated measurements in the data [@Scutari2017].
To obtain the structure, continuous variables suffixed with _1 were subtracted from variables suffixed with _2 to calculate the difference between variables measured at time points T1 $\rightarrow$ T2 and T2 $\rightarrow$ T3. 
