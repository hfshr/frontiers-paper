...
In addition to the log scaled cumulative values, an untransformed negative life event score from the first time point was included as an additional variable based on previous literature that indicates this variable should have a strong relationship with injury outcome [@Ivarsson2017].
...
The "individual or team sport" node had strong arcs to hours per week (`r str_finder("ind_team", "hours")`) and baseline negative life events (NLE) (`r str_finder("ind_team", "nlebase")`).
...
The arc from baseline NLE (nlebase) $\rightarrow$ Reward Interest (RI\_1) had a strength of `r str_finder("nlebase", "RI_1")`, and the probability of RI\_1 being in the "High" state increased from `r x[2,1]` to `r x[3,1]` when nlebase increased from "Low" to "High".
